import torch
import os
from network.base_net import BCTRANSFORER
...
class MABCQT:
    def __init__(self, args):
        ...
        # 神经网络
        self.eval_rnn = BCTRANSFORER(input_shape, args)  # 每个agent选动作的网络
        self.target_rnn = BCTRANSFORER(input_shape, args)
    ...
    def get_q_values(self, batch, max_episode_len):
        ...
        for transition_idx in range(max_episode_len):
            ...
            inputs_next_L.append(inputs_next)

        # shape(96, max_episode_len，42(obs))
        inputs_L = torch.stack(inputs_L, dim=1)
        inputs_next_L = torch.stack(inputs_next_L, dim=1)

        q_eval , i, imt = self.eval_rnn(inputs_L)
        q_target, _, _ = self.target_rnn(inputs_next_L)

        # 把q_eval维度重新变回(8, 5,n_actions)
        q_eval = q_eval.view(episode_num, max_episode_len, self.n_agents, -1)
        q_target = q_target.view(episode_num, max_episode_len, self.n_agents, -1)
        imt = imt.view(episode_num,max_episode_len, self.n_agents,  -1)
        i = i.view(episode_num,max_episode_len, self.n_agents,  -1)
        # 得的q_eval和q_target是一个列表，列表里装着max_episode_len个数组，数组的的维度是(episode个数, n_agents，n_actions)
        # 把该列表转化成(episode个数, max_episode_len， n_agents，n_actions)的数组

        return q_eval, q_target, imt, i
    ...
    def save_model(self, train_step):
        ...
        torch.save(self.eval_qmix_net.state_dict(), self.model_dir + '/' + num + '_mabcqt_net_params.pkl')
        torch.save(self.eval_rnn.state_dict(),  self.model_dir + '/' + num + '_trans_net_params.pkl')
